...
<div class="site-container">
    ...
    <section class="page_categorias">
        ...
        $readCat = new Read;
        $readCat->ExeRead("fwd_posts", "WHERE post_status = 1 AND (post_category = :cat OR post_cat_parent = :cat) ORDER BY post_date DESC LIMIT :limit OFFSET :offset", "cat={$category_id}&limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");
        if (!$readCat->getResult()):
            $Pager->ReturnPage();
            FWDErro("Desculpe, a categoria {$category_title} ainda não tem artigos publicados, favor volte mais tarde!", FWD_INFOR);
        else:
            $cc = 0;
            $View = new View;
        ...
        $Pager->ExePaginator("fwd_posts", "WHERE post_status = 1 AND (post_category = :cat OR post_cat_parent = :cat)", "cat={$category_id}");
