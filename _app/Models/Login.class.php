...
class Login {
    ...
    /**
     * <b>Obter Erro:</b> Retorna um array associativo com uma mensagem e um tipo de erro FWD_.
    ...
    private function setLogin() {
        if (!$this->Email || !$this->Senha || !Check::Email($this->Email))://Se não existir o email entra na menssagem abaixo
            $this->Error = ['Informe seu E-mail e senha para efetuar o login!', FWD_INFOR];
            $this->Result = false;
        elseif (!$this->getUser()):
            $this->Error = ['Os dados informados não são compatíveis!', FWD_ALERT];
            $this->Result = false;
        elseif ($this->Result['user_level'] < $this->Level):
            $this->Error = ["Desculpe {$this->Result['user_name']}, você não tem permissão para acessar esta área!", FWD_ERROR];
            $this->Result = false;
        else:
            $this->Execute();
        endif;
    ...
    //Vetifica usuário e senha no banco de dados! Se quiser criar metodo de recuperar senha esse é o local
    private function getUser() {
        $this->Senha = md5($this->Senha);

        $read = new Read;
        $read->ExeRead("fwd_users", "WHERE user_email = :e AND user_password = :p", "e={$this->Email}&p={$this->Senha}");
    ...
    private function Execute() {
        ...
        $this->Error = ["Olá {$this->Result['user_name']}, seja bem vindo(a). Aguarde redirecionamento!", FWD_ACCEPT];
        $this->Result = true;
    }
