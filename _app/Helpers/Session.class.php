...
class Session {
    ...
    private function setTraffic() {
        ...
        if (!$this->Traffic):
            ...
            $createSiteViews->ExeCreate('fwd_siteviews', $ArrSiteViews);
        else:
            if (!$this->getCookie()):
            ...
            $updateSiteViews->ExeUpdate('fwd_siteviews', $ArrSiteViews, "WHERE siteviews_date = :date", "date={$this->Date}");
    ...
    private function TrafficUpdate() {
        ...
        $updatePageViews->ExeUpdate('fwd_siteviews', $ArrSiteViews, "WHERE siteviews_date = :date", "date={$this->Date}");
    ...
    private function getTraffic() {
        ...
        $readSiteViews->ExeRead('fwd_siteviews', "WHERE siteviews_date = :date", "date={$this->Date}");
    ...
    private function getCookie() {
        ...
        setcookie("useronline", base64_encode("fwd"), time() + 86400);
        if (!$Cookie):
            return false;
        else:
            return true;
    ...
    private function BrowserUpdate() {
        $readAgent = new Read;
        $readAgent->ExeRead('fwd_siteviews_agent', "WHERE agent_name = :agent", "agent={$this->Browser}");
        if (!$readAgent->getResult()):
            $ArrAgent = ['agent_name' => $this->Browser, 'agent_views' => 1];
            $createAgent = new Create;
            $createAgent->ExeCreate('fwd_siteviews_agent', $ArrAgent);
        else:
            $ArrAgent = ['agent_views' => $readAgent->getResult()[0]['agent_views'] + 1];
            $updateAgent = new Update;
            $updateAgent->ExeUpdate('fwd_siteviews_agent', $ArrAgent, "WHERE agent_name = :name", "name={$this->Browser}");
        endif;
    }
    ...
    private function setUsuario() {
        ...
        $userCreate->ExeCreate('fwd_siteviews_online', $sesOnline);
    }
    ...
    private function UsuarioUpdate() {
        ...
        $userUpdate->ExeUpdate('fwd_siteviews_online', $ArrOnlone, "WHERE online_session = :ses", "ses={$_SESSION['useronline']['online_session']}");

        if (!$userUpdate->getRowCount()):
            $readSes = new Read;
            $readSes->ExeRead('fwd_siteviews_online', 'WHERE online_session = :onses', "onses={$_SESSION['useronline']['online_session']}");
