<div class="content home">
    ...
    <aside>
        ...
        <article class="sitecontent boxaside">
            ...
            //VISITAS DO SITE
            $read->FullRead("SELECT SUM(siteviews_views) AS views FROM fwd_siteviews");
            $Views = $read->getResult()[0]['views'];

            //USUÁRIOS
            $read->FullRead("SELECT SUM(siteviews_users) AS users FROM fwd_siteviews");
            $Users = $read->getResult()[0]['users'];

            //MÉDIA DE PAGEVIEWS
            $read->FullRead("SELECT SUM(siteviews_pages) AS pages FROM fwd_siteviews");
            $ResPages = $read->getResult()[0]['pages'];
            $Pages = substr($ResPages / $Users, 0, 5);

            //POSTS
            $read->ExeRead("fwd_posts");
    ...
    <section class="content_statistics">
        ...
        <section>
            ...
            <?php
            $read->ExeRead("fwd_posts", "ORDER BY post_date DESC LIMIT 3");
            if ($read->getResult()):
        ...
        <section>
            ...
            <?php
            $read->ExeRead("fwd_posts", "ORDER BY post_views DESC LIMIT 3");
            if ($read->getResult()):
                foreach ($read->getResult() as $re):
                    extract($re);
                    ?>
